...
const fetchItemsToCart = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while fetching items from cart" });
  }
};
...
const updateCartItems = async (req, res) => {
  try {
    ...
    if (!existingCart) {
      return res
        .status(404)
        .json({ message: "Cart not found for the user", key: 0 });
    }
    ...
    existingCart.items.forEach((existingItem) => {
      ...
      const newItem = cart.find(
        (newItem) =>
          newItem.foodItem.toString() === existingItem.foodItem.toString()
      );
      ...
      if (newItem) {
        ...
        existingCart.items = existingCart.items.filter(
          (item) =>
            item.foodItem.toString() !== existingItem.foodItem.toString()
        );
      }
    });
    ...
    cart.forEach((newItem) => {
      const existingItem = existingCart.items.find(
        (item) => item.foodItem.toString() === newItem.foodItem.toString()
      );
      if (!existingItem) {
    ...
    return res
      .status(200)
      .json({ message: "Cart items updated successfully", key: 1 });
  } catch (error) {
    console.error("Error updating cart items:", error);
    return res
      .status(500)
      .json({ message: "An error occurred while updating cart items", key: 0 });
  }
};
...
const restaurantFoodUpload = async (req, res) => {
  try {
    ...
    if (isMultiple) {
      ...
      res.json({
        key: 1,
        message: `${result.length} food items stored successfully`,
      });
    } else {
...
const generateRestaurantId = async () => {
  ...
  while (exists) {
    ...
    exists = existingRestaurant !== null;
  }
...
const restaurantUpload = async (req, res) => {
  try {
    const restaurantData = req.body.restaurants; // Assuming the request body contains restaurant information or an array of restaurant information
      // If the request body contains single restaurant informationge
      const updatedRestaurantData = [...restaurantData, {restaurantId:generateRestaurantId()}];
      const newRestaurant = new Restaurant(updatedRestaurantData);
...
const fetchRestaurantOrders = async (req, res) => {
  const { email } = req.query;
  const restaurant = await Restaurant.findOne({ email });
  const restaurantId = restaurant.restaurantId;
  //assuming that the person has already logged in and has thier items in the database uploaded
  if (email == null) {
  ...
  await Promise.all(
    orders.map((order) => {
      order.cart.map(async (array) => {
        if (array.foodItem.restaurantId == restaurantId) {
          const client = await Client.findOne({ email: order.userEmail });
...
const viewRestaurantItem = async () => {
  const {email} = req.query;
  const restaurant = await Restaurant.findOne({email});
  const foodItems = await Food.find({restaurantId:restaurant.restaurantId});

  res.json({message:"successfully fetched the orders", key:1, foodItems})
}
